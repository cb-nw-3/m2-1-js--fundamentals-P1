...
A comment is a line of code which doesn't do anything.
...
*this is not always as helpful as it sounds.
...
"I have" + 1 gives "I have 1"

but with backtics you can add numeric operations

`I have ${1+1 +'hello'}` gives "I have 2 hello"
...
A value has been explicitly set to an empty value (eg. "Not Applicable")
...
Double equals (==) ignores type so 5=="5" is true, but we do not use it !!!
...
5 && 3 when two values are true it takes the value on the right. 
...
They are not equal to false, but when you evalute them they are false. 
...
2. How do we access the value `true`? anArrayOfNumbers[3] or anArrayOfNumbers[anArrayOfNumbers.lenght - 1]
...
Numbers: 5 NaN 23 0 7 -99
Strings: 'hello' 'cat' '42'
Booleans: true false
Arrays: ['mouse', 'cat', 'dog'] ['coconut', 24, 'banana']
Object: {name: 'bob', age: 23} {product: 'cake', price: '50$'}
Empty Value: null unddefined
```
...
1. person.name Bob
2. person['name'] Bob
3. person[name] Undefined because it drops the variable name in person['John'] which is undefined 
...
[Next lecture: Expressions and Statements](../lecture-3-expressions-and-statements)
